...
class HttpManagementProbe(BatchingProbe):
    """
    A Probe implementation that sends requests to EAP's HTTP management interface.
    """

    def __init__(self, tests = []):
        super(HttpManagementProbe, self).__init__(tests)
    ...
    def sendRequest(self, context, request):
        url = "http://%s:%s/%s" % (self.host, self.port, context)
